...
.build_template: &build_definition
  ...
  - if [ -z "$VARIANT_TAG" ] && echo "VARIANT_TAG is empty" && exit 1
  - if [ -z "$VARIANT_TAG_WITH_VERSION" ] && echo "VARIANT_TAG_WITH_VERSION is empty" && exit 1
  - if [ -z "$VARIANT_BUILD_DIR" ] && echo "VARIANT_BUILD_DIR is empty" && exit 1
